...
由于当前cdh-hive-1.1.0 中的ORC与datax中parquet版本存在版本冲突，因此：在原hdfsparquetwriter基础上，另行开发插件支持parquet。该插件支持parquet和text格式，不支持parquet，使用时请注意


## 2 功能与限制

* (1)、目前HdfsWriter仅支持textfile和parquet两种格式的文件，且文件内容存放的必须是一张逻辑意义上的二维表;
        ...
        "content": [
            {
                "reader": {
                    "name": "txtfilereader",
                    "parameter": {
                        "path": ["/Users/shf/workplace/txtWorkplace/job/dataparquetfull.txt"],
                        "encoding": "UTF-8",
                        "column": [
                            {
                                "index": 0,
                ...
                "writer": {
                    ...
                    "parameter": {
                        ...
                        "fileType": "parquet",
                        "path": "/user/hive/warehouse/writerparquet.db/parquetfull",
                        "fileName": "xxxx",
                        "column": [
                            {
                                "name": "col1",
...
* **fileType**
	...
	* 描述：文件的类型，目前只支持用户配置为"text"或"parquet"。 <br />
...
* **writeMode**
 	...
 	* 描述：hdfsparquetwriter写入前数据清理处理模式： <br />

		* append，写入前不做任何处理，DataX hdfsparquetwriter直接使用filename写入，并保证文件名不冲突。
...
* **fieldDelimiter**
	...
	* 描述：hdfsparquetwriter写入时的字段分隔符,**需要用户保证与创建的Hive表的字段分隔符一致，否则无法在Hive表中查到数据** <br />
...
* **compress**
	...
	* 描述：hdfs文件压缩类型，默认不填写意味着没有压缩。其中：text类型文件支持压缩类型有gzip、bzip2;parquet类型文件支持的压缩类型有NONE、SNAPPY（需要用户安装SnappyCodec）。 <br />
...
```json
create database IF NOT EXISTS hdfsparquetwriter;
use hdfsparquetwriter;
...
STORED AS TEXTFILE;
```
text_table在hdfs上存储路径为：/user/hive/warehouse/hdfsparquetwriter.db/text_table/

（2）建立存储为parquet文件类型的表
```json
create database IF NOT EXISTS hdfsparquetwriter;
use hdfsparquetwriter;
...
STORED AS ORC;
```
parquet_table在hdfs上存储路径为：/user/hive/warehouse/hdfsparquetwriter.db/parquet_table/
